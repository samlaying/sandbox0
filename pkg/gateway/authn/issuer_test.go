...
func TestIssueTokenPair_UsesUniqueSessionIDs(t *testing.T) {
	...
	first, err := issuer.IssueTokenPair("user-1", "team-1", "admin", "user@example.com", "User", false)
	if err != nil {
		t.Fatalf("issue first token pair: %v", err)
	}
	second, err := issuer.IssueTokenPair("user-1", "team-1", "admin", "user@example.com", "User", false)
	if err != nil {
